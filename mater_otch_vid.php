...
";
		for ($z=0;$z<$cp;$z++)
		{
			if ($dt['$z][y']==$i)
			{	
				if ($dt['$z][m']==$months['$j][n'])	echo "<a href='mater_otch_vid.php?id=".$dt['$z][n']."' class='small'>Выдача №".$dt['$z][n']."(".$dt['$z][dt'].")</a><br>";
			}
		}
		echo "</td>";
	}
    echo "</tr></table>";
		...
		if (isset($_GET['id']))
		{
			$query = "SELECT `mesta_hr`.`nazv`, `sotr`.`surname`, `sotr`.`name`, `sotr`.`otch`
FROM mesta_hr, sotr, vid_mater
WHERE ((`mesta_hr`.`id` =`vid_mater`.`mesto_hr`) AND (`sotr`.`id` =`vid_mater`.`mol`) AND (`vid_mater`.`id` ='".$_GET['id']."'))";
			////////echo $query."<br />";
			$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
			$row = mysqli_fetch_array($result);
			echo "<div class=\"head2\">
			Место хранения: \"".$row['nazv']."\"<br>
			МОЛ: ".$row['surname']." ".$row['name']." ".$row['otch']."
			</div>";
			$query = "SELECT `mater`.`naim`, `mater_vid`.`Price`, `mater_vid`.`Kol-vo`
FROM mater, mater_vid
WHERE ((`mater`.`id` =`mater_vid`.`mater`) AND (`mater_vid`.`vid_mater` ='".$_GET['id']."'))";
			////////echo $query."<br />";
			$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);				
