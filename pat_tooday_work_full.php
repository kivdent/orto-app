...
if (($_GET['perv']==1) or ($_GET['SodNazn']==4) or ($_GET['action']=="osm"))
{
	...
	switch ($_GET['step'])
	{
		...
		//////сохранение в медкарте
			...
			//echo $query."<br>";
			if ($f==1) $result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
			$f=0;
...
echo "                </table></td>
	...
	$result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
	$row = mysqli_fetch_array($result);
	if ($NZub==1) echo $row['Nazv']."</div>";
	else  echo $NZub."-й зуб, ".$row['Nazv']."</div>";
...
if ($_SESSION['QZub']==$_GET['count'])	echo "<input name='step' type='hidden' value='4'/>";
else
{
...
<center><input name='next' type='submit'  value='Дальше>>>'/></center>
	...
	for ($i=1;$i<=$_SESSION['countm'];$i++)
	{
		if ($_GET['chek']==$_SESSION['chek'][$i][1])
		{
			...
			else $_SESSION['chek'][$i][2]=$_SESSION['chek'][$i][2]-1;
		}
	}
	ret("pat_tooday_work_full.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk);
	break;
	...
	for ($i=1;$i<=$_SESSION['QZub'];$i++)
		{
			...
			$result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
			$row = mysqli_fetch_array($result);
			if ($NZub==1) $ds= $row['Nazv'];
	else  $ds=$NZub."-й зуб, ".$row['Nazv'];
			echo $ds;
		}		
...
for ($i=0;$i<$count;$i++)
{
	...
	else echo "|<a class=menu2 href='pat_tooday_work_full.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$row['id']."'>".$row['preysk']."</a>|";
}
		echo " </center></td>
	  </tr>
	  <tr>
...
$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$preysk." order by `range`, `manip`";
	...
	case "5":
		...
		for ($i=1;$i<=$_SESSION['QZub'];$i++)
		{
		if ($_SESSION['NZub'][$i]==1) $zh.=$_SESSION['zh'][$i]."<br />";
		...
		for ($i=1;$i<=$_SESSION['QZub'];$i++)
		{
		if ($_SESSION['NZub'][$i]==1) $an.=$_SESSION['an'][$i]."<br />";
		...
		for ($i=1;$i<=$_SESSION['QZub'];$i++)
		{
		if ($_SESSION['NZub'][$i]==1) $obk.=$_SESSION['obk'][$i]."<br />";
		...
		for ($i=1;$i<=$_SESSION['QZub'];$i++)
		{
			...
			$result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
			$row = mysqli_fetch_array($result);
			if ($NZub==1) $ds=$ds.$row['Nazv']."<br />";
			else $ds=$ds.$NZub."-й зуб, ".$row['Nazv']."<br />";
...
//		
		$query = "INSERT INTO `manip_pr` (`id`, `NZuba`, `manip`, `kolvo`, `dnev`) VALUES";
			...
			{
				if ($j==1)$query.="(NULL,'0','".$_SESSION['chek'][$j][1]."','".$_SESSION['chek'][$j][2]."','".$pr."')";
				else $query.=",(NULL,'0','".$_SESSION['chek'][$j][1]."','".$_SESSION['chek'][$j][2]."','".$pr."')";
			}
		...
		for ($i=1;$i<=$_SESSION['countm'];$i++)
		{
			if ($i==1) $query.=$_SESSION['chek'][$i][1];
			else $query.=", ".$_SESSION['chek'][$i][1];
		}
		//include("footer2.php");
		$query.=")";
