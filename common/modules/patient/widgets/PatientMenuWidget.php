...
//            ['label' => 'Ортопедия', 'url' => ['/old_app/pat_card.php', 'id' => $this->patient_id, 'action' => 'ortp']],
            ...
            ['label' => 'Назначения', 'url' => ['/patient/appointments/', 'patient_id' => $this->patient_id]],
            ['label' => 'Фотографии', 'url' => ['/photos/', 'patient_id' => $this->patient_id]],

        ];
        if (UserInterface::isUserRole(UserInterface::ROLE_ADMIN)
            || UserInterface::isUserRole(UserInterface::ROLE_SENIOR_RECORDER)
            || UserInterface::isUserRole(UserInterface::ROLE_RECORDER)){
            $this->menuItems[]= ['label' => 'Коммуникатор', 'url' => ['/patient/notifier/', 'patient_id' => $this->patient_id]];
        }
