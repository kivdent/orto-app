	...
	for ($i=0;$i<$count;$i++)
{
	...
	if ($_SESSION['nzub']==$row['id']) echo "<option selected='selected' value='pat_tooday_work.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&chek=".$_SESSION['chek'][$_GET['count']][$i][1]."nzub=".$row['id']."'>".$row['NZuba']."</option>";
	else  echo "<option value='pat_tooday_work.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&chek=".$_SESSION['chek'][$_GET['count']][$i][1]."&nzub=".$row['id']."'>".$row['NZuba']."</option>";
...
//		$row = mysqli_fetch_array($result);
		...
		VALUES (NULL, '".$_SESSION["UserID"]."','".$_SESSION['pat']."', '".date('Y-m-d')."', '0', ' ', ' ', ' ', ' ', ' ', 0,'".$opl."','".$summ_sk."',0,".$_SESSION['proc_sk'].",".$_SESSION['nzub'].",".$_SESSION['Nid'].")" ;
		echo $query."<br>";
