	  ...
	  <tr>
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($row['id'] == $preysk)
                ...
                echo "|<a class=menu2 href='pat_tooday_work.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $row['id'] . "'>" . $row['preysk'] . "</a>|";
        }
        echo " </center></td>
	  </tr>
	  <tr>
...
//		$row = mysqli_fetch_array($result);
		...
		VALUES (NULL, '" . $_SESSION["UserID"] . "','" . $_SESSION['pat'] . "', '" . date('Y-m-d') . "', '0', ' ', ' ', ' ', ' ', ' ', 0,'" . $opl . "','" . $summ_sk . "',0," . $_SESSION['proc_sk'] . ",'0'," . $_SESSION['Nid'] . ")";
        echo $query . "<br>";
        $result = sql_query($query, 'orto', 0);
...
//		
        ...
        for ($i = 1; $i <= 1; $i++) {
            ...
            for ($j = 1; $j <= $_SESSION['countm'][$i]; $j++) {
                if ($j == 1)
                    $query .= "(NULL,'0','" . $_SESSION['chek'][$i][$j][1] . "','" . $_SESSION['chek'][$i][$j][2] . "','" . $pr . "')";
                else
                    $query .= ",(NULL,'0','" . $_SESSION['chek'][$i][$j][1] . "','" . $_SESSION['chek'][$i][$j][2] . "','" . $pr . "')";
            }
        }
