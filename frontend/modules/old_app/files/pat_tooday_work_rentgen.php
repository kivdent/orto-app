...
//	}
        ...
        switch ($_GET['act']) {
            case "add":
                ...
                ret("pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk);
                break;
            case "del":
			...
			function ChQ(id,qq)
			{
				...
				url='pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $_GET['preysk'] . "&id='+id+'&act=chQ&sstep=2&q='+q;
				location.href=url;
			}";
                    echo "ChQ('" . $_GET['id'] . "','" . $_SESSION['chek'][$_GET['count']][$_GET['id']][2] . "')</script>";
                } else {
                    $_SESSION['chek'][$_GET['count']][$_GET['id']][2] = $_GET[q];
                    ret("pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $_GET['preysk']);
                }
                break;
            case "next":
                if (isset($_SESSION['countm'][$_GET['count']])) {
                    ret("pat_tooday_work_rentgen.php?action=lech&step=5&count=" . $_GET['count'] . "&ass=" . $_GET['ass']);
                    //exit;
                    echo "<form id='lechf' name='lechf' method='get' action='pat_tooday_work_rentgen.php'>
            <label></label>
            Правка лечения:<br />
...
//			}
            ...
            if (!(isset($_GET['preysk']))) $preysk = 4000;
            else $preysk = $_GET['preysk'];
            if (!(isset($_SESSION['pat']))) $_SESSION['pat'] = $_GET['pat'];
            if (isset($_GET['pat'])) $_SESSION['pat'] = $_GET['pat'];
            //////////Заполнение лечения
            echo "<form action='pat_tooday_work_rentgen.php' method='get' id='lech' name='lech'>
...
<select name=\"ass\">";
            ...
            for ($j = 0; $j < $asstCount; $j++) {
                if ($asst[$j]['id'] == $_GET['ass']) echo " <option value='" . $asst[$j]['id'] . "' selected='selected'>" . $asst[$j]['name'] . "</option>";
                else echo " <option value='" . $asst[$j]['id'] . "' >" . $asst[$j]['name'] . "</option>";
            }
            echo "</select>";
...
//			echo "</select> <br />";
			  ...
			  <tr>
				...
				<a href='pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk . "&chek=" . $_SESSION['chek'][$_GET['count']][$i][1] . "&act=del' class='niz2'>Удалить из списка</a>
</td>
				<td width='10%' align='center'>" . $_SESSION['chek'][$_GET['count']][$i][2] . "<br />
<a href='pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk . "&id=" . $i . "&act=chQ&sstep=1' class=niz2>изменить</a> </td>
    ...
    ";
                    ...
                    for ($j = 1; $j <= $cm; $j++) {
                        ...
                        if ($cat[$i]['id'] == $mat[$j]['UpId'])
                            echo "<tr>
        <td width='85%'><a href='pat_tooday_work_rentgen.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk . "&manip=" . $mat[$j]['id'] . "&act=add' class='small'>" . $mat[$j]['manip'] . "</a></td>
        <td width='15%'>
		" . $mat[$j]['price'] . "
		</td>
      </tr>";
...
//		$row = mysqli_fetch_array($result);
		...
		VALUES (NULL, '" . $_GET['ass'] . "','" . $_SESSION['pat'] . "', '" . date('Y-m-d') . "', '0', ' ', ' ', ' ', ' ', ' ', 0,'" . $opl . "','" . $summ_sk . "',0," . $_SESSION['proc_sk'] . ",0)";
        //echo $query."<br>";
        $result = sql_query($query,'orto',0);;
...
//;
        echo "<a href='rentgen.php'class='mmenu'>Закрыть</a>";
        unset($_SESSION['chek']);
