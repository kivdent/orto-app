...
switch ($_GET['type'])
{
	...
	echo "| <a class='menu2' href='dir_den_opl_per_2010.php?fp=".$row['id']."&type=vrach_otch&vrach=".$_GET['vrach']."' >".$m[($dt['1]-1)']." ".$dt[0]."</a> |";
}
...
echo "</table>";
	...
	$summ2=0;
	$tables=array ("dnev","zaknar","schet_orto");
	$query = "select * from preysk where id!=1000 order by id";
	//echo $query."<br />";
	$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
	$resultP=$result;
	$countP=$count;
	...
	$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$rowP['id']." order by `range`, `manip`";
		//echo $query."<br />";
		$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
		echo $rowP['preysk']."<br>";
		$cc=0;
...
$countW=$count;
			for ($w=0;$w<$countW;$w++)
			{
				...
				else
				{
					$cm++;
					$mat['$cm][id']=$rowW['id'];
					$mat['$cm][manip']=$rowW['manip'];
					$mat['$cm][price']=$rowW['price'];
					$mat['$cm][UpId']=$rowW['UpId'];
					$mat['$cm][kolvo']=0;
					$mat['$cm][total']=0;
$manip1=array("manip_pr","manip_zn","manip_sh_orto");
$manip2=array("`manip_pr`.`dnev`","`manip_zn`.`ZN`","`manip_sh_orto`.`SO`");
for ($j=0;$j<=2;$j++)
		{
	$query = "SELECT SUM(`".$manip1['$j']."`.`kolvo`) as kolvo, `manip`.`price`,(SUM(`$manip1['$j']`.`kolvo`)* `manip`.`price`) as total,`manip`.`id`
	FROM ".$manip1['$j'].", manip,".$tables['$j']."
	WHERE (
	(`manip`.`id` =`".$manip1['$j']."`.`manip`) AND 
	(".$manip2['$j']." =`".$tables['$j']."`.`id`)AND
...
for ($i=0;$i<$count;$i++)
	{
						...
						if ($row['id']==$mat['$cm][id'])
						{
						$mat['$cm][kolvo']+=$row['kolvo'];
						$mat['$cm][total']+=$row['total'];
						$summ+=$row['total'];
						
						}
	}
}
$mat['$cm][new_price']=round((($mat['$cm][total']*1.079)/$mat['$cm][kolvo']),-1);
$summ2+=($mat['$cm][new_price']*$mat['$cm][kolvo']);		
$mat['$cm][new_tot']=($mat['$cm][new_price']*$mat['$cm][kolvo']);
}
...
for ($i=0; $i<$count; $i++)
{
	...
	$dt=explode("-",$row['okonch']);
	echo "| <a class='menu2' href='dir_den_opl_per_2010.php?fp=".$row['id']."' >".$m[($dt['1]-1)']." ".$dt[0]."</a> |";
}
...
for ($i=0;$i<$c;$i++)
{
	...
	echo "<tr>
    <td><a href=\"dir_den_opl_per_2010.php?type=vrach_otch&vrach=".$sotr_sp['$i']."\">".$sotr[$sotr_sp['$i]][name']."</a></td>
...
echo "<table width='100%' border='1' cellpadding='0' cellspacing='0' bordercolor='#000000'>
					  <tr>
					  <td width='40%'><div align='center' class='feature3'>id</div></td>
						<td width='40%'><div align='center' class='feature3'>Наименование</div></td>
						<td width='20%'><div align='center' class='feature3'>Количество</div></td>
						<td width='20%'><div align='center' class='feature3'>Цена</div></td>
						<td width='20%'><div align='center' class='feature3'>Стоимость</div></td>
...
for ($j=0;$j<=2;$j++)
		{
	$query = "SELECT `manip`.`manip`,SUM(`".$manip1['$j']."`.`kolvo`) as kolvo, `manip`.`price`,(SUM(`$manip1['$j']`.`kolvo`)* `manip`.`price`) as total,`manip`.`id`
	FROM ".$manip1['$j'].", manip,".$tables['$j']."
	WHERE (
	(`manip`.`id` =`".$manip1['$j']."`.`manip`) AND 
	(".$manip2['$j']." =`".$tables['$j']."`.`id`)AND
...
for ($i=0;$i<$count;$i++)
	{
	$row= mysqli_fetch_array($result);
 echo "<tr><td width='40%'><div align='left' class='feature'>".$row['id']."</div></td>
						<td width='40%'><div align='left' class='feature'>".$row['manip']."</div></td>
						<td width='20%'><div align='center' class='feature3'>".$row['kolvo']."</div></td>
						<td width='20%'><div align='center' class='feature3'>".$row['price']."</div></td>
						<td width='20%'><div align='center' class='feature3'>".$row['total']."</div></td>
