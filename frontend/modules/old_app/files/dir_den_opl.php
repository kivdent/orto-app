...
if ($_SESSION["valid_user"]=="director") echo "<h4><div class='head3'>". $prev."|".date("d-m-Y",$DayPr).", ".$dn[(date("w",$DayPr)-1)]."|".$next."</div></h4>";
else echo "<h4><div class='head3'>|".date("d-m-Y",$DayPr).", ".$dn[(date("w",$DayPr)-1)]."|</div></h4>";
$query = "SELECT `kassa`.`timeN`, `kassa`.`timeO`, `kassa`.`summ`, `sotr`.`surname`, `sotr`.`name`, `sotr`.`otch`,`kassa`.`id`
FROM kassa, sotr
WHERE ((`kassa`.`date` ='".date('Y-m-d',$DayPr)."') AND 
(`sotr`.`id` =`kassa`.`sotr` )) 
...
$rowA= mysqli_fetch_array($resultA);
	...
	for ($j=0;$j<=2;$j++)
	{
		...
		WHERE (
		(`oplata`.`date` ='".date('Y-m-d',$DayPr)."') AND 
			  ...
			  for ($i=0;$i<$count;$i++)
				{
					...
					$opl[$h][$opl[$h]['kol']]['patN']="<a class='mmenu' target='_blanc' href=\"show.php?type=chek&dnev=".$row[15]."&table=".$tables[$j]."&podr=".$opl[$h]['id']."\">".$opl[$h][$opl[$h]['kol']]['patN']."</a>";
...
$query = "SELECT 
		...
		`oplata`.`VidOpl`,
		`schet_hyg`.`id`
		FROM oplata, schet_hyg, opl_vid
		WHERE (
		(`oplata`.`date` ='".date('Y-m-d',$DayPr)."') AND 
		(`oplata`.`dnev` =`schet_hyg`.`id`) AND 
		(`oplata`.`VidOpl`=`opl_vid`.`id`)
		 AND
		(`oplata`.`type`=5)
		)
		GROUP BY `oplata`.`podr`";
		//echo $query."<br />";
		//$result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
		$summ['hyg'][1]=0;
		$summ['hyg'][2]=0;
...
WHERE (
(`pr_avans`.`date` ='".date('Y-m-d',$DayPr)."') AND  
...
echo "<span class='head3'>Сумма за день: ".($summ['pr_av']+$summ[2]['obsch']+$summ['hyg'][1])."</span><br />";
if ($summ['hyg'][1]!=0) echo "<span class='head3'>Продажа гигиены за день: ".$summ['hyg'][1]."</span><br />";
...
if ($summ[2]['dog']!=0) echo "<span class='head3'>По договорам за день: ".$summ[2]['dog']."</span><br />";
if ($summ[2]['bank_card']!=0) echo "<span class='head3'>По банковским картам: ".$summ[2]['bank_card']."</span><br />";
if ($summ[2]['proc']!=0) echo "<span class='head3'>Бесплатная гигиена по 10% картам: ".$summ[2]['proc']."</span><br />";

if ($summ[2]['av']!=0) echo "<span class='head3'>Из авансов за день: ".$summ[2]['av']."</span><br />";
if (($summ['kassa']+$summ['sn_kass'][1])<=($summ['pr_av']+$summ[2]['nal'])) 
{
	echo "<span class='head3'>Выручка к сдаче: ".($summ['pr_av']+$summ[2]['nal']+$summ['kassa2']-$summ['kassa']-$summ['sn_kass'][1])."</span><br />";
//msg($summ['pr_av']."+".$summ[2]['nal']."+".$summ['kassa2']."-".$summ['sn_kass'][1]."-".$summ['kassa']."=".($summ['pr_av']+$summ[2]['nal']+$summ['kassa2']-$summ['kassa']-$summ['sn_kass'][1]));
}
else 
{
	
	echo "<span class='head3'>Выручка к сдаче: ".($summ['pr_av']+$summ[2]['nal']+$summ['kassa2']-$summ['sn_kass'][1])."</span><br />";
	//msg($summ['pr_av']."+".$summ[2]['nal']."+".$summ['kassa2']."-".$summ['sn_kass'][1]."=".($summ['pr_av']+$summ[2]['nal']+$summ['kassa2']-$summ['sn_kass'][1]));

}
echo "<a class='mmenu' target='_blanc' href=\"show.php?type=otch_d&podr=1\">Печать отчёта  ".$opl['2][nazv']."</a><br />";
echo "<hr width='100%' noshade='noshade' size='1'/>";

echo "<span class='head1'>Итоги: ".$opl[1]['nazv']."</span><br />";		
echo "<span class='head3'>Сумма за день: ".($summ[1]['obsch']+$summ['hyg'][2])."</span><br />";
if ($summ['hyg'][2]!=0) echo "<span class='head3'>Продажа гигиены за день: ".$summ['hyg'][2]."</span><br />";
if ($summ[1]['dog']!=0) echo "<span class='head3'>По договорам за день: ".$summ[1]['dog']."</span><br />";
if ($summ[1]['av']!=0) echo "<span class='head3'>Из авансов за день: ".$summ[1]['av']."</span><br />";
if ($summ[1]['bank_card']!=0) echo "<span class='head3'>По банковским картам: ".$summ[2]['bank_card']."</span><br />";
if ($summ[1]['proc']!=0) echo "<span class='head3'>Бесплатная гигиена по 10% картам: ".$summ[2]['proc']."</span><br />";
...
echo "<a class='mmenu' target='_blanc' href=\"show.php?type=otch_d&podr=2\">Печать отчёта  ".$opl['1][nazv']."</a><br />";
echo "<hr width='100%' noshade='noshade' size='1'/>";
