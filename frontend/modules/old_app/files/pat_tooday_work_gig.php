...
case "4":
	...
	ret("pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk);
	break;
	case "del":
	...
	case "chQ":
		...
		{	
			...
			function ChQ(id,qq)
			{
				...
				url='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$_GET[preysk]."&id='+id+'&act=chQ&sstep=2&q='+q;
				location.href=url;
			}";
		...
		else
		{
			...
			ret("pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$_GET[preysk]);
		}
	break;
	case "next":
	if (isset($_SESSION[countm][$_GET[count]]))
		{
		ret("pat_tooday_work_gig.php?action=lech&step=5&count=".$_GET[count]);
		exit;
					echo "<form id='lechf' name='lechf' method='get' action='pat_tooday_work_gig.php'>
            <label></label>
            Правка лечения:<br />
...
//			}
	...
	//////////Заполнение лечения
	echo "<form action='pat_tooday_work_gig.php' method='get' id='lech' name='lech'>
...
for ($i=0;$i<$count;$i++)
{
	...
	else echo "|<a class=menu2 href='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$row[id]."'>".$row[preysk]."</a>|";
}
		echo " </center></td>
	  </tr>
	  <tr>
...
//			echo "</select> <br />";
			...
			for ($i=1;$i<=$_SESSION[countm][$_GET[count]];$i++)
			{
				...
				<a href='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&chek=".$_SESSION[chek][$_GET[count]][$i][1]."&act=del' class='niz2'>Удалить из списка</a>
</td>
				<td width='10%' align='center'>".$_SESSION[chek][$_GET[count]][$i][2]."<br />
<a href='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&id=".$i."&act=chQ&sstep=1' class=niz2>изменить</a> </td>
...
function MM_jumpMenu2(targ,selObj,restore){
  eval(targ+\".location='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&skidka=\"+selObj.options[selObj.selectedIndex].value+\"'\");
...
$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$preysk." order by range, manip";
    ...
    ";
				...
				for($j=1;$j<=$cm;$j++)
				{
					...
					echo "<tr>
        <td width='85%'><a href='pat_tooday_work_gig.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&manip=".$mat[$j][id]."&act=add' class='small'>". $mat[$j][manip]."</a></td>
        <td width='15%'>
		".$mat[$j][price]."
		</td>
      </tr>";
...
for ($i=1;$i<=1;$i++)
		...
		$query = "INSERT INTO `dnev` (`id`, `vrach`,`pat`, `date`, `osm`, `ds`, `zh`, `an`, `obk`, `lech`, `resl`,`summ`, `summ_k_opl`, `summ_vnes`,`skidka`)
		VALUES (NULL, '".$_SESSION["UserID"]."','".$_SESSION[pat]."', '".date('Y-m-d')."', '0', ' ', ' ', ' ', ' ', ' ', 0,'".$opl."','".$summ_sk."',0,".$_SESSION[proc_sk].")" ;
		//////////echo $query."<br>";
		include("query.php");
