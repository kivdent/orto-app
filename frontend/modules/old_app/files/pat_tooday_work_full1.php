...
<center><input name='next' type='submit'  value='Дальше>>>'/></center>
	...
	for ($i=1;$i<=$_SESSION[countm][$_GET[count]];$i++)
	{
		...
		{
			for ($j=$i;$j<$_SESSION[countm][$_GET[count]];$j++)
			{
	...
	for ($i=1;$i<=$_SESSION[QZub];$i++)
		{
			...
			include("query.php");
			$row = mysql_fetch_array($result);
			if ($NZub==1) $ds= $row[Nazv]."</div>";
	else  $ds=$NZub."-й зуб, ".$row[Nazv]."</div>";
			echo $ds;
		}		
...
//				echo "<option value=".$row[id].">".$N."</option>";
//				$_SESSION[summ]=$_SESSION[summ]+($row[price]*$_SESSION[chek][$_GET[count]][$i][2]);
//			}
//			echo "</select> <br />";
			...
			for ($i=1;$i<=$_SESSION[countm][$_GET[count]];$i++)
			{
				...
				<a href='pat_tooday_work_full.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&chek=".$_SESSION[chek][$_GET[count]][$i][1]."&act=del' class='niz2'>Удалить из списка</a>
</td>
...
<a href='pat_tooday_work_full.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&id=".$i."&act=chQ&sstep=1' class=niz2>изменить</a> </td>
			...
			$_SESSION[summ]+=$_SESSION[chek][$_GET[count]][$i][2]*$_SESSION[chek][$_GET[count]][$i][4];	
			} 
			echo "</table>";
			echo "<div align='right'>Итого: ".$_SESSION[summ]." руб. </div>";
////скидка
...
function MM_jumpMenu2(targ,selObj,restore){
  eval(targ+\".location='pat_tooday_work_full.php?action=lech&step=4&count=".$_GET[count]."&preysk=".$preysk."&skidka=\"+selObj.options[selObj.selectedIndex].value+\"'\");
  if (restore) selObj.selectedIndex=0;
}
//-->
</script>
			...
			for ($i=0;$i<$count;$i++)
			{
				...
				if ($ck==$row[id]) 
				{
					...
					if (round(($_SESSION[summ]-($_SESSION[summ]*$row[proc])/100),-1)==$_SESSION[summ])
					{
						$summ_sk=(floor((($_SESSION[summ]-($_SESSION[summ]*$row[proc])/100))/10))*10;
					}
					else
					{
						$summ_sk=round(($_SESSION[summ]-($_SESSION[summ]*$row[proc])/100),-1);
					}
				}
...
$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$preysk." order by manip";
	...
	case "5":
		...
		$zh=$_SESSION[NZub][$i]." ".$_SESSION[an][$i]."<br />";	
		}
		echo $an."<br />";
		echo "Объективно: ";
		for ($i=1;$i<=$_SESSION[QZub];$i++)
		...
		$zh=$_SESSION[NZub][$i]." ".$_SESSION[obk][$i]."<br />";		
		}
		echo $obk."<br />";
		echo "Диагноз : ";
		for ($i=1;$i<=$_SESSION[QZub];$i++)
...
//		}
		...
		VALUES (NULL, '".$_SESSION["UserID"]."','".$_SESSION[pat]."', '".date('Y-m-d')."', '0', ' ', ' ', ' ', ' ', ' ', 0,'".$opl."','".$summ_sk."',0,".$_SESSION[proc_sk].",".$_SESSION[Nid].")" ;
		echo $query."<br>";
		include("query.php");
