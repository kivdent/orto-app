...
//echo "</select><input name='action' type='hidden' value='del' />";
	echo "<table width='100%' border='1' cellpadding='0' cellspacing='0' bordercolor='#000000'>
			...
			for ($i=1;$i<=$_SESSION['QEl'];$i++)
			{
				...
				<td align='center'><a href=\"mater_vid.php?action=del&prih=".$_SESSION['Prih']['$i][id']."\" class='menu2'>Удалить</a>
				</br><a href=\"mater_vid.php?action=ch&prih=".$_SESSION['Prih']['$i][id']."\" class='menu2'>Изменить количество</a></td>
			  </tr>";
...
//	}
	...
	<table width='80%' border='0'>";
				...
				for($j=1;$j<=$_SESSION['cm'];$j++)
				{
					if ($_SESSION['cat']['$i][id']==$_SESSION['mat']['$j][UpId'])
					echo "<tr>
        <td width='85%'><a href='mater_vid.php?action=add&id=".$_SESSION['mat']['$j][id']."&varr=".$_SESSION['mat']['$j][varr']."&qp=".$_SESSION['mat']['$j][qpack']."&max=".$_SESSION['mat']['$j][ost']."&price=".$_SESSION['mat']['$j][Price']."' class='small'>". $_SESSION['mat']['$j][naim']."</a></td>
        <td>".$_SESSION['mat']['$j][ost']." 
			".$_SESSION['mat']['$j][abbr']."</td>
      </tr>";
      }
				echo "</table></div>";
			}
