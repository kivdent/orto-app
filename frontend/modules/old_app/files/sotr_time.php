...
switch ($_GET['action'])
{
	case "set_time_in":
		...
		{
			case "1":
				$query = "INSERT INTO `tabel_reg` (`id`, `sotr`, `date`, `in`,`out`)
				VALUES (NULL, ".$_GET['sotr'].", '".date("Y-m-d")."', '".date("H:i").":00', '00:00:00')";
				//echo $query;
...
for ($q=1;$q<=date("t"); $q++)
{
	if ((date('D',(mktime(0,0,0,date('m'),$q,date('Y'))))=='Sun') or (date('D',(mktime(0,0,0,date('m'),$q,date('Y'))))=='Sat')) $bg="bgcolor='#999999'";
	else $bg="";
	echo "<td class='bottom' ".$bg.">".$q." ".date('d',$q)."</td>";
	//echo "<td>".$q."</td>";
}
...
for ($j=1;$j<=$count;$j++)
{
	...
	for ($i=1;$i<=date("t"); $i++)
{
	if ((date('D',(mktime(0,0,0,date('m'),$i,date('Y'))))=='Sun') or (date('D',(mktime(0,0,0,date('m'),$i,date('Y'))))=='Sat')) $bg="bgcolor='#999999'";
	else $bg="";
...
// остатльные
default:
if (!(isset($_GET['action']))) ret("sotr_time.php?action=month");
$mn=array("Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь","Октябрь","Ноябрь","Декабрь");
echo "<center>|<a href=\"sotr_time.php?action=all\" class=\"menu2\">Все пациенты</a>|";
echo "<br>";
...
for ($i=0;$i<=11;$i++)
{
	...
	echo "|<a href=\"sotr_time.php?action=month&month=".($i+1)."&year=".$year."\" class=\"".$cl."\">".$mn[$i]."</a>|";
	if ($i==5) echo "<br>";
}
echo "</td>
<td align='left'>";
for ($i=2007;$i<=(date('Y')+2);$i++)
{
	...
	echo "|<a href=\"sotr_time.php?action=month&month=".$month."&year=".$i."\" class=\"".$cl."\">".$i."</a>|";
...
echo "
	...
	$query = "SELECT `nach` , `okonch` 
FROM `fin-per` 
WHERE ((`nach`<='".$year."-".$month."-".date('d')."') and(`okonch`>='".$year."-".$month."-".date('d')."'))
ORDER BY `id` DESC 
LIMIT 0,1" ;
...
for ($i=0;$i<$count;$i++)
{
	...
	///$dt2=(string)mktime(0,0,0,$dt[1],$dt[2],$dt[0]);
	//$dt2=$row['date'];
	...
	//else $dt2+==60*60*24;
	//$dt2=(integer)$dt[2];	
	$dt2=(integer)($dt[1].$dt[2]);
	//echo $row['date']." ".$dt2."</br>";
	$in=explode(":",$row['in']);
	$in2=mktime($in[0],$in[1],0,date('m'),date('d'),date('Y'));
...
//	else $bg="";
	...
	if (date("Y-m-d")==date("Y-m-d",$i))
	{
		...
		if ($r[$row['id']][$dt]['tm']>0)
		{
			...
			$summ+=$r[$row['id']][$dt]['tm'];	echo "<td class='bottom'  width='20' align='center' valign='middle' ".$bg.">".$r[$row['id']][$dt]['in']."<br>".$r[$row['id']][$dt]['out']."<br>".(floor($r[$row['id']][$dt]['tm']/3600))." ч: ".((($r[$row['id']][$dt]['tm'])-floor($r[$row['id']][$dt]['tm']/3600)*3600)/60)." м <br>
			</td>";
		}
		else 
		{
