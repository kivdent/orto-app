...
$query = "SELECT `id`, `surname`, `name`, `otch` FROM `sotr` WHERE `dolzh`=5 ORDER BY surname" ;
			...
			echo "<form method=\"get\"  name=\"assf\"><script type=\"text/JavaScript\">
<!--
function MM_jumpMenu(targ,selObj,restore){
  eval(targ+\".location='doc_den_opl_xray_per.php?ass=\"+selObj.options[selObj.selectedIndex'].value+\"'\");
  if (restore) selObj.selectedIndex=0;
}
//-->
</script>";
			echo "Ассистент 
<select name=\"ass\" onchange=\"MM_jumpMenu('parent',this,0)\">";
for ($j=0;$j<$asstCount;$j++)
  {
		if (($j==0) and (!isset($_GET['ass']))) $sotr=$asst[$j]['id'];
	  if ($asst[$j]['id']==$_GET['ass'])  
	  {
		  		echo " <option value='".$asst[$j]['id']."' selected='selected'>".$asst[$j]['name']."</option>";
		  		 $sotr=$asst[$j]['id'];
		  	}
