    ...
    switch ($_GET['step']) {
        case "1":
            ...
            echo "		<div class='head3' >Осмотр пациента</div>
							<a href='pat_tooday_work_full.php?action=lech&step=1&osmID&pat=" . $_SESSION['pat'] . "' class='menu2'>Пропустить</a>
					<form id='form1' name='form1' method='get' action='pat_tooday_work_full.php'>
        ...
        case "2":
            ...
            for ($i = 0; $i < $_GET['CountAll']; $i++) {
                if ($AllVid[$i] != "") {
                    if ($f == 0)
                        $query = $query . " (NULL, " . $MCID . "," . $AllVid[$i] . ", " . $AllProyav[$i] . ") ";
                    else
                        $query = $query . ", (NULL,'" . $MCID . "','" . $sopzabID[$i] . "')";
                    $f = 1;
                }
            }
            //echo $query."<br>";
...
ORDER BY `sz`.`sz` ASC";
            ...
            for ($i = 0; $i < $count; $i++) {
                $row = mysqli_fetch_array($result);
                if ($row['id'] == 10)
                    echo "<input name='sz' type='radio' value='" . $row['id'] . "' checked='checked'/>" . $row['sz'] . "<br />";
                else
...
WHERE (`nzuba`.`id` ='" . ($_GET['ZubN'] + 1) . "')";
	...
	ORDER BY `sz`.`sz` ASC";
        ...
        case "4":
            ...
            echo "<a href='pat_tooday_work_full.php?action=lech&step=1&osmID&pat=" . $_SESSION['pat'] . "' class='menu2'>Начать лечение.</a>";
        ...
        switch ($_GET['step']) {
            case "1":
                ...
                if (!(isset($_SESSION['OsmID']))) {
					...
					<a href='pat_tooday_work_full.php?action=osm&step=1&pat=" . $_SESSION['pat'] . "&perv=0' class='mmenu'>Провести осмотр</a>";
                        $_SESSION['OsmID'] = 0;
                    }
                }
...
////Фамилия
                ...
                $count = mysqli_num_rows($result);
                if ($count > 0) {
                    echo "<div class='feature5'>У пациента аллергия: ";
                    for ($i = 0; $i < $count; $i++) {
                        $row = mysqli_fetch_array($result);
...
";
	...
	WHERE ((`advid`.`id` =`medcard`.`ADVid`) AND (`medcard`.`PatID`  ='" . $_SESSION['pat'] . "'))";
                ...
                echo "		<form  action='pat_tooday_work_full.php' method='get' >
...
";
		...
		<input name='action' type='hidden' value='lech' />
		<input name='step' type='hidden' value='3'/>
		<input name='NZub[1]' type='hidden' value='1'/><div align='center'>";
                    echo "Выбор диагноза<br />";
                    $_SESSION['QZub'] = 1;
				...
				ORDER BY `ds`.`KlassID` ASC';
                    //echo $query.'<br>';
                    include('query.php');
                    echo "<select name='dsZub[1]' size='" . $count . "' >";
                    for ($i = 0; $i < $count; $i++) {
                        $row = mysqli_fetch_array($result);
		...
		<input name='Input' type='submit'  value='Дальше>>>'/></div>  
            ...
            case "3":
                switch ($_GET['act']) {
                    case "next":
                        if (!(isset($_SESSION['zh'][$_GET['count']])))
                            $_SESSION['zh'][$_GET['count']] = $_GET['zh_1'];
                        if (!(isset($_SESSION['an'][$_GET['count']])))
                            $_SESSION['an'][$_GET['count']] = $_GET['an_1'];
                        if (!(isset($_SESSION['obk'][$_GET['count']])))
                            $_SESSION['obk'][$_GET['count']] = $_GET['obk_1'];
                        ret("pat_tooday_work_full.php?action=lech&step=3&count=" . ($_GET['count'] + 1));
                        break;
                }
                ////////Заполнение карты
                //msg($_GET['NZub']);
                if (!(isset($_SESSION['NZub'])))
                    $_SESSION['NZub'] = $_GET['NZub'];
                if (!(isset($_SESSION['dsZub'])))
                    $_SESSION['dsZub'] = $_GET['dsZub'];
                if (isset($_GET['lech']))
                    $_SESSION['lech'][$_GET['count']] = $_GET['lech'];
                $NZub = $_SESSION['NZub'][$_GET['count']];
                //$NZub=$NZub1;
                $dsZub = $_SESSION['dsZub'][$NZub];
                //msg($_SESSION['NZub']." ".$_SESSION['dsZub']);
	...
	<input name='obk_1' type='hidden' value='" . $_GET['count'] . "' />";
                            ...
                            for ($i = 0; $i < $count1; $i++) {
                                ...
                                echo "
					<a href='pat_tooday_work_full.php?action=lech&step=3&count=" . $_GET['count'] . "&klishe=" . $row1['id'] . "' class='menu2'>" . $row1['nazv'] . "</a><div class=\"smalltext\">
" . stripslashes($row1['text']) . "</div><hr width='100%' noshade='noshade' size='1'/>";
                ...
                //$dsZub=$dsZub;
                echo "<form method='get' action='pat_tooday_work_full.php' id='card' name='card'>
	<input name='count' type='hidden' value='" . $_GET['count'] . "' />
	 <div class='head3'> Пациент:" . $_SESSION['pat_name'] . "</div>";
                if ($_SESSION['QZub'] == $_GET['count'])
                    echo "<input name='step' type='hidden' value='4'/>";
                else {
                    echo "<input name='step' type='hidden' value='3'/>
	<input name='act' type='hidden' value='next'/>";
                }
...
ORDER BY `zh`.`zh` ASC";
                ...
                if ($count > 25)
                    echo "<select id='tablefields' name='var_zh' size='1' multiple='multiple' ondblclick='insertValueQuery()' onMouseOver='document.card.var_zh.size=25' onmouseout='document.card.var_zh.size=1'>";
                else
                    echo "<select id='tablefields' name='var_zh' size='1' multiple='multiple' ondblclick= 'insertValueQuery()' onMouseOver='document.card.var_zh.size=" . $count . "' onmouseout='document.card.var_zh.size=1'>";
                for ($i = 0; $i < $count; $i++) {
                    $row = mysqli_fetch_array($result);
...
ORDER BY `an`.`an` ASC";
                ...
                if ($count > 25)
                    echo "<select id='tablefields' name='var_an' size='1' multiple='multiple' ondblclick='insertValueQueryan()' onMouseOver='document.card.var_an.size=25' onmouseout='document.card.var_an.size=1'>";
                else
                    echo "<select id='tablefields' name='var_an' size='1' multiple='multiple' ondblclick= 'insertValueQueryan()' onMouseOver='document.card.var_an.size=" . $count . "' onmouseout='document.card.var_an.size=1'>";
  ...
  <tr>
	...
	ORDER BY `ob`.`ob` ASC";
                    ...
                    if ($count > 25)
                        echo "<select id='tablefields' name='var_obk' size='1' multiple='multiple' ondblclick='insertValueQueryobk()' onMouseOver='document.card.var_obk.size=25' onmouseout='document.card.var_obk.size=1'>";
                    else
                        echo "<select id='tablefields' name='var_obk' size='1' multiple='multiple' ondblclick= 'insertValueQueryobk()' onMouseOver='document.card.var_obk.size=" . $count . "' onmouseout='document.card.var_obk.size=1'>";
...
<center><input name='next' type='submit'  value='Дальше>>>'/></center>
		...
		LIMIT 1";
                ...
                switch ($_GET['act']) {
                    case "add":
                        if (!(isset($_SESSION['countm']))) {
                            ...
                            $row = mysqli_fetch_array($result);
                            $_SESSION['countm'] = 1;
                            $_SESSION['chek'][$_SESSION['countm']][1] = $_GET['manip'];
                            $_SESSION['chek'][$_SESSION['countm']][2] = 1;
                            $_SESSION['chek'][$_SESSION['countm']][3] = $row['manip'];
                            $_SESSION['chek'][$_SESSION['countm']][4] = $row['price'];
                        } else {
                            $f = 0;
                            for ($i = 1; $i <= $_SESSION['countm']; $i++) {
                                if ($_GET['manip'] == $_SESSION['chek'][$i][1]) {
                                    $f = 1;
                                    $_SESSION['chek'][$i][2] = $_SESSION['chek'][$i][2] + 1;
                                }
                            }
                            if ($f == 0) {
                                ...
                                $count = mysqli_num_rows($result);
                                $row = mysqli_fetch_array($result);
                                $_SESSION['countm'] = $_SESSION['countm'] + 1;
                                $_SESSION['chek'][$_SESSION['countm']][1] = $_GET['manip'];
                                $_SESSION['chek'][$_SESSION['countm']][2] = 1;
                                $_SESSION['chek'][$_SESSION['countm']][3] = $row['manip'];
                                $_SESSION['chek'][$_SESSION['countm']][4] = $row['price'];
                                $_SESSION['chek'][$_SESSION['countm']][5] = $row['zapis'];
                            }
                        }
                        ret("pat_tooday_work_full.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk);
                        break;
                    case "del":
                        if ($_SESSION['countm'] == 1) {
                            $_SESSION['countm'] = 0;
                        } else
                            for ($i = 1; $i <= $_SESSION['countm']; $i++) {
                                if ($_GET['chek'] == $_SESSION['chek'][$i][1]) {
                                    for ($j = $i; $j < $_SESSION['countm']; $j++) {

                                        $_SESSION['chek'][$j][1] = $_SESSION['chek'][$j + 1][1];
                                        $_SESSION['chek'][$j][2] = $_SESSION['chek'][$j + 1][2];
                                        $_SESSION['chek'][$j][3] = $_SESSION['chek'][$j + 1][3];
                                        $_SESSION['chek'][$j][4] = $_SESSION['chek'][$j + 1][4];
                                        $_SESSION['chek'][$j][5] = $_SESSION['chek'][$j + 1][5];
                                    }
                                    $_SESSION['countm'] = $_SESSION['countm'] - 1;
                                    $i = $j + 1;
                                }
			...
			function ChQ(id,qq)
			{
				...
				url='pat_tooday_work_full.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $_GET['preysk'] . "&id='+id+'&act=chQ&sstep=2&q='+q;
				location.href=url;
			}";
                            echo "ChQ('" . $_GET['id'] . "','" . $_SESSION['chek'][$_GET['id']][2] . "')</script>";
                        } else {
                            $_SESSION['chek'][$_GET['id']][2] = $_GET[q];
                            ret("pat_tooday_work_full.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $_GET['preysk']);
                        }
                        break;
                    case "next":
                        if (isset($_SESSION['countm'])) {
                            echo "<form id='lechf' name='lechf' method='get' action='pat_tooday_work_full.php'>
            <label></label>
            Правка лечения:<br />
...
//				$N=$N."_";
//				}
//				$N=$N.$row['price']."*".$_SESSION['chek'][$i][2]."=".($row['price']*$_SESSION['chek'][$i][2])."руб.";
//				echo "<option value=".$row['id'].">".$N."</option>";
...
//			echo "</select> <br />";
			  ...
			  <tr>
				...
				<td width='10%' align='center'>" . $_SESSION['chek'][$i][2] . "<br />
<a href='pat_tooday_work_full.php?action=lech&step=4&count=" . $_GET['count'] . "&preysk=" . $preysk . "&id=" . $i . "&act=chQ&sstep=1' class=niz2>изменить</a> </td>
...
//		}
                ...
                $opl = $opl + $_SESSION['summ'];
                if (round(($_SESSION['summ'] - ($_SESSION['summ'] * $_SESSION['proc_sk']) / 100), -1) == $_SESSION['summ']) {
                    $summ_sk = (floor((($_SESSION['summ'] - ($_SESSION['summ'] * $_SESSION['proc_sk']) / 100)) / 10)) * 10;
                } else {
                    $summ_sk = round(($_SESSION['summ'] - ($_SESSION['summ'] * $_SESSION['proc_sk']) / 100), -1);
                }
                $query = "INSERT INTO `dnev` (`id`, `vrach`,`pat`, `date`, `osm`, `ds`, `zh`, `an`, `obk`, `lech`, `resl`,`summ`, `summ_k_opl`, `summ_vnes`,`skidka`,`Nid`)
		VALUES (NULL, '" . $_SESSION["UserID"] . "','" . $_SESSION['pat'] . "', '" . date('Y-m-d') . "', '0', ' ', '" . $zh . "', '" . $an . "', '" . $obk . "', '" . $lech . "', 0,'" . $opl . "','" . $summ_sk . "',0," . $_SESSION['proc_sk'] . "," . $_SESSION['Nid'] . ")";
                //echo $query."<br>";
                $result = sql_query($query, 'orto', 0);
                $pr = $result;
                ...
                for ($i = 1; $i <= $_SESSION['QZub']; $i++) {
                    ...
                    $dsZub = $_SESSION['dsZub'][$NZub];
                    if ($i == 1) {
                        $query = $query . "(NULL,'" . $dsZub . "','" . $pr . "','" . $NZub . "')";
                    } else
                        $query = $query . ", (NULL,'" . $dsZub . "','" . $pr . "','" . $NZub . "') ";
                }
...
//		
                ...
                for ($j = 1; $j <= $_SESSION['countm']; $j++) {
                    if ($j == 1)
                        $query .= "(NULL,'0','" . $_SESSION['chek'][$j][1] . "','" . $_SESSION['chek'][$j][2] . "','" . $pr . "')";
                    else
                        $query .= ",(NULL,'0','" . $_SESSION['chek'][$j][1] . "','" . $_SESSION['chek'][$j][2] . "','" . $pr . "')";
                }
			  ...
			  <tr>
                ...
                for ($i = 1; $i <= $_SESSION['countm']; $i++) {
                    echo "  <tr>
				<td width='6%' align='center'>" . $i . "</td>
				<td width='62%' align='left'>" . $_SESSION['chek'][$i][3] . "</td>
				<td width='10%' align='center'>" . $_SESSION['chek'][$i][2] . "</td>
				<td width='12%' align='center'>" . $_SESSION['chek'][$i][4] . " руб.</td>
				<td width='10%' align='center'>" . ($_SESSION['chek'][$i][2] * $_SESSION['chek'][$i][4]) . " руб.</td>
			  </tr>";
                    $s += $_SESSION['chek'][$i][2] * $_SESSION['chek'][$i][4];
                }
...
//	break;
    ...
    if (($_GET['action'] != "lech")) {
        echo "<script language='JavaScript' type='text/javascript'>
		location.href='pat_tooday_work_full.php?action=lech&perv=0&step=1&pat=" . $_GET['pat'] . "';
		</script>";
    }
}
