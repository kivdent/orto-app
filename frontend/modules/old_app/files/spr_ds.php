	...
	if ($countA>0)
	{
		...
		<td><table width='100%' border='0' cellspacing='0' cellpadding='1'>
		  <tr>
			...
			<td width='25%' align='center'><a href='spr_ds.php?mod=show&action=change&id=".$rowA['id']."&cat=1&klass=".$klass."&step=1' class='niz2'>Изменить</a></td>
			<td width='25%' align='center'><a href='spr_ds.php?mod=show&action=del&id=".$rowA['id']."&cat=1&klass=".$klass."' class='niz2' >Удалить</a></td>
		  </tr>
		</table></td>
	  </tr>";
...
WHERE ((`ds`.`KlassID` ='".$klass."') AND (`ds`.`upID` ='".$upID."') AND (`ds`.`Cat` =0))";
    ...
    <td><table width='100%' border='0' cellspacing='0' cellpadding='1'>
      <tr>
        <td width='".($ur*$tab)."'><img src='image/transrerent.gif'  width='".($ur*$tab)."' height=1/></td>
        <td width='50%' class='head2' >".$rowB['Nazv']."</td>
        <td width='25%' align='center'><a href='spr_ds.php?mod=show&action=change&id=".$rowB['id']."&cat=0&klass=".$klass."&step=1' class='niz2'>Изменить</a></td>
        <td width='25%' align='center'><a href='spr_ds.php?mod=show&action=del&id=".$rowB['id']."&cat=0&klass=".$klass."' class='niz2'>Удалить</a></td>
      </tr>
    </table></td>
  </tr>";
			}
...
echo "  <tr>
    <td><table width='100%' border='0' cellspacing='0' cellpadding='1'>
      <tr>
         <td width='".($ur*$tab)."'><img src='image/transrerent.gif'  width='".($ur*$tab)."' height=1/></td>
        <td width='100%' align='left'><a href='spr_ds.php?mod=show&action=add&upID=".$upID."&step=1&cat=1&klass=".$klass."' class='niz2'>Добавить подраздел </a></td>
        </tr>
    </table>
   ...
   <tr>
    <td><table width='100%' border='0' cellspacing='0' cellpadding='1'>
      <tr>
         <td width='".($ur*$tab)."'><img src='image/transrerent.gif'  width='".($ur*$tab)."' height=1/></td>
        <td width='100%' align='left'><a href='spr_ds.php?mod=show&action=add&upID=".$upID."&step=1&cat=0&klass=".$klass."' class='niz2'>Добавить диагноз</a> </td>
        </tr>
    </table>
...
FROM razd";
	...
	for ($i=0;$i<$countA;$i++)
	{
		...
		if ($count>0)
		{
			...
			{
				$rowB = mysqli_fetch_array($resultB);
				echo "<a class='mmenu' href='spr_ds.php?mod=show&action=show&klass=".$rowB['id']."'>".$rowB['Nazv']."</a>
				 -- <a href='spr_ds.php?mod=klass&action=del&klass=".$rowB['id']."&razd=".$rowB['Razd']."' class='mmenu'>Удалить</a><br>";	
			}
		}
		else echo "<div class='head2'>Нет классификаций в разделе</div>";
		echo "<a href='spr_ds.php?mod=klass&action=add&razd=".$rowA['id']."&step=1' class='mmenu'>Добавить классификацию в этот раздел</a>";
	}
}
///////////////////////////////Начало работы
//////Выбор модуля
	...
	case "klass":
		switch ($_GET['action'])
		{
			...
			////////////////////Пошаговое добавление
				...
				{
					case "1":
						echo "<form action='spr_ds.php' method='get' name='f1' id='f1'>
