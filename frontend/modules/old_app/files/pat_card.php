...
$ro = isset($_GET['ro']) ? $ro = $_GET['ro'] : false;
$id = $_GET['id'];
...
<select name=\"ko\" onchange=\"MM_jumpMenu2('parent',this,0)\">";
                for ($i = 0; $i < $count; $i++) {
                    $row = mysqli_fetch_array($result);
                    if (!(isset($_GET['ko'])) and ($i == 0)) {
                        $ko = $row['id'];
                        $next_date = $row['next_date'];
                    }
                    if ((isset($_GET['ko'])) and ($row['id'] == $_GET['ko'])) {
...
GROUP BY
    `sotr`.`id`";
            ...
            $row = mysqli_fetch_array($result);
            if ($count > 0) {
                $dt = mktime(0, 0, 0, $row['m'], $row['d'], $row['y']);
            } else {
                $dt = mktime(0, 0, 0, 1, 1, 1990);
            }
            $fp[$j] = $dt;
...
GROUP BY
    `sotr`.`id`";
        ...
        $row = mysqli_fetch_array($result);
        IF ($count > 0) {
            echo "Номер карты бесплатной гигиены " . $row['id'] . "<br><br>";
        }
