...
{
	case "add":
		$query = "INSERT INTO manip (`id`, `manip`, `preysk`, `zapis`, `price`)
					VALUES (NULL,'".$_GET['manip']."','".$_GET['preysk']."','".$_GET['zapis']."','".$_GET['price']."')";
		//////////echo $query."<br />";
		$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
...
else echo "<select name='id' size='".$count."'>";
if (!($count>0))
{
	$N="Название";
	while (strlen($N)<60) 
...
echo "</select><br />
<input name='change' type='submit'  value='Изменить'  onclick='document.manipf.action.value=\"change\"'/>
<input name='del' type='submit'  value='Удалить' onclick='document.manipf.action.value=\"del\"'/></center>
</td>
          </tr>
        </table></form>";
//include("footer.php");
