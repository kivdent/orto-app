...
case "4":
	...
	ret("pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk);
	break;
	case "del":
	...
	case "chQ":
		...
		{	
			...
			function ChQ(id,qq)
			{
				...
				url='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$_GET['preysk']."&id='+id+'&act=chQ&sstep=2&q='+q;
				location.href=url;
			}";
		...
		else
		{
			...
			ret("pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$_GET['preysk']);
		}
	break;
	case "next":
	if (isset($_SESSION['countm'][$_GET['count']]))
		{
		ret("pat_tooday_work_mat.php?action=lech&step=5&count=".$_GET['count']);
		exit;
					echo "<form id='lechf' name='lechf' method='get' action='pat_tooday_work_mat.php'>
            <label></label>
            Правка лечения:<br />
...
//			}
	...
	if (!(isset($_GET['preysk']))) $preysk="4001";
	else $preysk=$_GET['preysk'];
	if (!(isset($_SESSION['pat']))) $_SESSION['pat']=$_GET['pat'];
	//////////Заполнение лечения
	echo "<form action='pat_tooday_work_mat.php' method='get' id='lech' name='lech'>
	...
	<div class='head2'>Номер зуба <select name=\"nzub\" onchange=\"javascript:location.href=this.options[this.selectedIndex].value\">";
	$query = "SELECT `id`,`NZuba` FROM `nzuba` order by `id`";
echo $query."<br>";
$result=sql_query($query,'orto',0);    $count=mysqli_num_rows($result);
if (isset($_GET['nzub']))  $_SESSION['nzub']=$_GET['nzub'];
	for ($i=0;$i<$count;$i++)
{
	$row = mysqli_fetch_array($result);
	if (($i==0) and (!(isset($_SESSION['nzub']))))
	{
		$_SESSION['nzub']=$row['id'];
	}
	
	if ($_SESSION['nzub']==$row['id']) echo "<option selected='selected' value='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&chek=".$_SESSION['chek'][$_GET['count']][$i][1]."nzub=".$row['id']."'>".$row['NZuba']."</option>";
	else  echo "<option value='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&chek=".$_SESSION['chek'][$_GET['count']][$i][1]."&nzub=".$row['id']."'>".$row['NZuba']."</option>";
...
for ($i=0;$i<$count;$i++)
{
	...
	else echo "|<a class=menu2 href='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$row['id']."'>".$row['preysk']."</a>|";
}
		echo " </center></td>
	  </tr>
	  <tr>
...
//			echo "</select> <br />";
			...
			for ($i=1;$i<=$_SESSION['countm'][$_GET['count']];$i++)
			{
				...
				<a href='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&chek=".$_SESSION['chek'][$_GET['count']][$i][1]."&act=del' class='niz2'>Удалить из списка</a>
</td>
				<td width='10%' align='center'>".$_SESSION['chek'][$_GET['count']][$i][2]."<br />
<a href='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&id=".$i."&act=chQ&sstep=1' class=niz2>изменить</a> </td>
			...
			echo "</table>";
			echo "<div align='right'>Итого: ".$_SESSION['summ'][$_GET['count']]." руб. <br>";
...
$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$preysk." order by `range`, `manip`";
    ...
    ";
				...
				for($j=1;$j<=$cm;$j++)
				{
					...
					echo "<tr>
        <td width='85%'><a href='pat_tooday_work_mat.php?action=lech&step=4&count=".$_GET['count']."&preysk=".$preysk."&manip=".$mat[$j]['id']."&act=add' class='small'>". $mat[$j]['manip']."</a></td>
        <td width='15%'>
		".$mat[$j]['price']."
		</td>
      </tr>";
...
//		$row = mysqli_fetch_array($result);
		...
		VALUES (NULL, '0','".$_SESSION['pat']."', '".date('Y-m-d')."', '0', ' ', ' ', ' ', ' ', ' ', 0,'".$opl."','".$opl."',0,0,".$_SESSION['nzub'].",".$_SESSION['Nid'].")" ;
		//////////echo $query."<br>";
