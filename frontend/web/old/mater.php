...
VALUES (NULL, '".$_GET['naim']."', '".$_GET['edizm']."', '".$QPack."', '".$QPrice."','".$Price."', '".$_GET['UpId']."', '0', '".$_GET['MinOst']."', '".$_GET['prim']."')";
				...
				$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
				$row = mysqli_fetch_array($result);
				$query = "INSERT INTO `ost_mat` (`id`, `mesto_hr`, `mater`, `ost`) VALUES (NULL, ".$osn.", ".$row[0].",0)";
				echo $query."<br />";
				$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
				$row = mysqli_fetch_array($result);
	...
	break;
	case "del":
		$query = "SELECT `id` FROM `mater` WHERE `UpId`=".$_GET['id'];
		////echo $query."<br />";
		$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
		if ($count>0)
		{
			...
			{
				$row = mysqli_fetch_array($result);
				$query = "DELETE FROM `ost_mat` WHERE `mater`=".$row['id'];
				////echo $query."<br />";
				$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
				$query = "DELETE FROM `mater` WHERE `id`=".$row['id'];
				////echo $query."<br />";
				$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
			}
		}
		$query = "DELETE FROM `ost_mat` WHERE `mater`=".$_GET['id'];
		//echo $query."<br />";
		$result=sql_query($query,'orto',0);     $count=mysqli_num_rows($result);
...
if ($count>0)
{
	...
	for ($i=0;$i<$count;$i++)
	{
		...
		for($i=1;$i<=$cc;$i++)
			{
				echo "
			<SPAN id='Out".$i."' class='mmenuHand'>".$cat['$i][naim']."</SPAN><br />
	<div id=Out".$i."details style=\"display:None; position:relative; left:12;\">
	<a href='mater.php?action=del&id=".$cat['$i][id']."&cat=1' class='small2'>Удалить категорию</a>|<a href='mater.php?action=AddMat&UpId=".$cat['$i][id']."&step=1' class='small2'>Добавить материал</a>
    ...
    ";
				...
				for($j=1;$j<=$cm;$j++)
				{
					...
					echo "<tr>
        <td width='85%'><a href='mater.php?action=change&id=".$mat['$j][id']."' class='small'>". $mat['$j][naim']."</a></td>
        <td width='15%'>
		".$mat['$j][price']."
		</td>
      </tr>";
