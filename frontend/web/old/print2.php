  ...
  <tr>
	...
	$query = "SELECT `manip`.`manip`, `manip_zn`.`kolvo`, `manip`.`price`, (`manip_zn`.`kolvo` *`manip`.`price`) AS STOIM
		...
		$query = "SELECT `klinikpat`.`surname`, 
						 ...
						 `schet_orto`.`summ`
				FROM klinikpat, schet_orto, sotr, skidka
...
$rowA= mysqli_fetch_array($resultA);
	...
	echo "<form action='dir_den.php' method='get' >
		<br /><center><span class='feature3' >Финансовый отчёт ".$opl['$h][nazv']." за  ".$dt2[2].".".$dt2[1].".".$dt2[0]."</span></center>
...
{	
	...
	else echo "<span class='bold2'>Выручка к сдаче: ".($summ['pr_av']+$summ['1][nal']+$summ['kassa2']-$summ['sn_kass][1'])."</span><br />";
}
else
{		
