...
include("header2.php");
	 switch ($_GET[action])
	{
		case "oplata":
			...
			ret("pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$preysk);
			break;
			case "del":
			...
			case "chQ":
				...
				{	
					...
					function ChQ(id,qq)
					{
						...
						url='pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$_GET[preysk]."&id='+id+'&act=chQ&sstep=2&q='+q;
						location.href=url;
					}";
				...
				else
				{
					...
					ret("pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$_GET[preysk]);
				}
			break;
			case "next":
...
WHERE ((`skidka`.`id` =`klinikpat`.`Skidka`) AND (`klinikpat`.`id` ='".$_SESSION[pat]."'))" ;
		...
		$query = "INSERT INTO `schet_orto` (`id`, `vrach`, `pat`, `date`, `summ`, `summ_k_opl`, `summ_vnes`)
		VALUES (NULL, 
		'".$_SESSION["UserID"]."',
		'".$_SESSION[pat]."', 
		'".date('Y-m-d')."',
		'".$opl."',
		'".round((($opl-($opl*$ck)/100)),-1)."',
		0)" ;
		////echo $query."<br>";
		include("query.php");
...
unset($_SESSION[chek]);
	...
	unset($_SESSION[summ]);	
		...
		for ($i=0;$i<$count;$i++)
		{
			...
			else echo "|<a class=menu2 href='pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$row[id]."'>".$row[preysk]."</a>|";
		}
				echo " </center>--></td>
			  </tr>
			  <tr>
		...
		//		  <input type='submit' name='Submit' value='Добавить в список' />";
				...
				if ($_SESSION[countm][$_GET[count]]>0)
				{
					...
					for ($i=1;$i<=$_SESSION[countm][$_GET[count]];$i++)
					{
						...
						<a href='pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$preysk."&chek=".$_SESSION[chek][$_GET[count]][$i][1]."&act=del' class='niz2'>Удалить из списка</a>
		</td>
						<td width='10%' align='center'>".$_SESSION[chek][$_GET[count]][$i][2]."<br />
		<a href='pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$preysk."&id=".$i."&act=chQ&sstep=1' class=niz2>изменить</a> </td>
		...
		$query = "select `id`, `manip`, `price`, `cat`, `UpId` from manip WHERE preysk=".$preysk." order by range, manip";
			...
			";
						...
						for($j=1;$j<=$cm;$j++)
						{
							...
							echo "<tr>
				<td width='85%'><a href='pat_tooday_work_orto.php?action=oplata&count=".$_GET[count]."&preysk=".$preysk."&manip=".$mat[$j][id]."&act=add' class='small'>". $mat[$j][manip]."</a></td>
				<td width='15%'>
				".$mat[$j][price]."
				</td>
			  </tr>";
...
WHERE ((`skidka`.`id` =`klinikpat`.`Skidka`) AND (`klinikpat`.`id` ='".$_SESSION[pat]."'))" ;
		...
		$query = "INSERT INTO `zaknar` (`id`, `vrach`, `pat` , `tech`, `date`, `dateSd`, `timeSd`, `summ`, `summ_k_opl`, `summ_vnes`)
		VALUES (NULL, 
		'".$_SESSION["UserID"]."',
		'".$_SESSION[pat]."', 
		'".$_GET[tech]."',
			...
			case "br":
				switch ($_SESSION[step])
				{
					case "1":
						...
						echo "Стоимость ретенционногj аппарата: <input name='RetApp' id='RetApp' type='text' />
